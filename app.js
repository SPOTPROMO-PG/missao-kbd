/* app.js — versão com cards da Home coloridos por status */
...
function renderHome(){
  ...
  CONTENT.marcas.forEach((m)=>{
    ...
    // ✅ CLASSE VISUAL
    card.className = `card brand-card ${prog.pct === 100 ? "done" : "pending"}`;
...
/* Quiz */
let quizState={ marcaAtual:null, kbdAtual:null, perguntaIndex:0, acertos:0, total:0, respondendo:false, perguntas:[] };
...
function renderQuiz(){
  ...
  if(!perguntas.length){
    document.getElementById("quizArea").innerHTML=`
      <div class="card">
        ...
        <div class="card-sub">Este KBD continua pendente.</div>
  ...
  quizState={ marcaAtual:marca, kbdAtual:kbd, perguntaIndex:0, acertos:0, total:perguntas.length, respondendo:false, perguntas };
  mostrarPergunta();
}
...
function findNextPendingKbdInBrand(marcaId){
  ...
  const data=getCompletedData();
  for(const kbd of marca.kbds){
    if(!(data[marcaId] && data[marcaId][kbd.id])) return {marca:marcaId, kbd:kbd.id};
  }
  return null;
}
...
function mostrarResultadoFinal(){
  ...
  const nextInBrand=findNextPendingKbdInBrand(quizState.marcaAtual.id);
  const nextBrand = nextInBrand ? null : findNextBrandWithPending(quizState.marcaAtual.id);
  ...
  document.getElementById("quizArea").innerHTML=`
    <div class="result">
      ...
      <div class="result-sub">${quizState.acertos} de ${quizState.total} perguntas corretas</div>
