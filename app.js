...
function entrar() {
  ...
  if (!ALLOWED_SECTORS.includes(normalized)) {
    alert(`Setor inválido. Use: ${ALLOWED_SECTORS.join(", ")}.`);
    return;
  }
...
function renderMarca() {
  ...
  applyTopbar({ logo: marca.logo, eyebrow: "Marca", title: marca.nome, subtitle: `${marca.kbds.length} KBDs disponíveis`, showBack: true, onBack: voltarHome, minimal: true, hideMenu: true, hideLogout: true });
...
function renderKbd() {
  ...
  applyTopbar({ logo: marca.logo, eyebrow: marca.nome, title: "Aula KBD", subtitle: kbd.nome, showBack: true, onBack: voltarMarca, minimal: true, hideMenu: true, hideLogout: true });
...
function renderQuiz() {
  ...
  applyTopbar({ logo: marca.logo, eyebrow: marca.nome, title: "Quiz", subtitle: kbd.nome, showBack: true, onBack: voltarKbd, minimal: true, hideMenu: true, hideLogout: true });
...
function normalizeBulletText(value) {
  let text = String(value || "").replace(/\s+/g, " ").replace(/\s+([,.;:!?])/g, "$1").replace(/([.;:!?])(?!\s|$)/g, "$1 ").trim();
