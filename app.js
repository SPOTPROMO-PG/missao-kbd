...
function renderQuiz() {
  ...
  // ✅ NOVO: QUIZ por MARCA + KBD
  // Estrutura esperada: QUIZZES[marcaId][kbdId] = [...]
  const byBrand = (window.QUIZZES && window.QUIZZES[marcaId]) || null;
  const perguntas =
    (byBrand && typeof byBrand === "object" && byBrand[kbdId]) ? byBrand[kbdId] : [];
...
function findNextPending() {
  ...
  for (const marca of CONTENT.marcas) {
    for (const kbd of marca.kbds) {
      if (!(data[marca.id] && data[marca.id][kbd.id])) return { marca: marca.id, kbd: kbd.id };
    }
  }
  return null;
}
