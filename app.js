...
const ALLOWED_SECTORS_NORMALIZED = new Set(
  ALLOWED_SECTORS.map((sector) => normalizeSector(sector))
);
...
function getQuizResultsData() {
  return JSON.parse(localStorage.getItem("QUIZ_RESULTS") || "{}");
}

function saveQuizResultsData(data) {
  localStorage.setItem("QUIZ_RESULTS", JSON.stringify(data));
}

function getSentBrandsData() {
  return JSON.parse(localStorage.getItem("BRANDS_SENT_TO_SHEETS") || "{}");
}

function saveSentBrandsData(data) {
  localStorage.setItem("BRANDS_SENT_TO_SHEETS", JSON.stringify(data));
}

function getMarcaById(marcaId) {
  return CONTENT.marcas.find((m) => m.id === marcaId) || null;
}

function getKbdById(marcaId, kbdId) {
  const marca = getMarcaById(marcaId);
  return marca ? marca.kbds.find((kbd) => kbd.id === kbdId) || null : null;
}

function getAllKbdsTotal() {
  return CONTENT.marcas.reduce((sum, marca) => sum + marca.kbds.length, 0);
}

function getAllKbdsDone() {
  const data = getCompletedData();
  return CONTENT.marcas.reduce(
    (sum, marca) => sum + marca.kbds.filter((kbd) => !!(data[marca.id] && data[marca.id][kbd.id])).length,
    0
  );
}
...
function getBrandThemeClass(marcaId) {
  return `theme-${marcaId || "default"}`;
}
...
function getBrandProgress(marcaId) {
  ...
  const done = marca.kbds.filter((kbd) => isQuizCompleted(marca.id, kbd.id)).length;
  return { done, total, pct: total ? Math.round((done / total) * 100) : 0 };
}

function getFirstPendingQuiz() {
  for (const marca of CONTENT.marcas) {
    const pending = marca.kbds.find((kbd) => !isQuizCompleted(marca.id, kbd.id));
    if (pending) return { marcaId: marca.id, kbdId: pending.id };
  }
  return null;
}

function getNextPendingInBrand(marcaId) {
  const marca = getMarcaById(marcaId);
  if (!marca) return null;
  const pending = marca.kbds.find((kbd) => !isQuizCompleted(marca.id, kbd.id));
  return pending ? { marcaId, kbdId: pending.id } : null;
}
...
function findNextBrandWithPending(afterMarcaId) {
  ...
  for (let i = idx + 1; i < CONTENT.marcas.length; i++) {
    if (getNextPendingInBrand(CONTENT.marcas[i].id)) return CONTENT.marcas[i].id;
  }
...
function getQuizQuestions(marcaId, kbdId) {
  const byBrand = (window.QUIZZES && window.QUIZZES[marcaId]) || {};
  return Array.isArray(byBrand[kbdId]) ? byBrand[kbdId] : [];
}

function saveKbdResult(marcaId, kbdId, payload) {
  const data = getQuizResultsData();
  if (!data[marcaId]) data[marcaId] = {};
  data[marcaId][kbdId] = payload;
  saveQuizResultsData(data);
}

function getBrandResults(marcaId) {
  const data = getQuizResultsData();
  return data[marcaId] || {};
}

function isBrandSentToSheets(marcaId) {
  const sent = getSentBrandsData();
  return !!sent[marcaId];
}

function markBrandSentToSheets(marcaId) {
  const sent = getSentBrandsData();
  sent[marcaId] = new Date().toISOString();
  saveSentBrandsData(sent);
}
  ...
  area.innerHTML = `
    <div class="result-card">
      ...
      <div class="result-stats">
        ...
        ${!nextInBrand ? `<span class="summary-chip completed">Envio da marca disparado</span>` : ""}
      </div>
...
function voltarMarca() {
  const marcaId = qs().get("marca");
  window.location.href = marcaId ? `marca.html?marca=${encodeURIComponent(marcaId)}` : "home.html";
}
...
function voltarKbd() {
  ...
  window.location.href =
    marcaId && kbdId
      ? `kbd.html?marca=${encodeURIComponent(marcaId)}&kbd=${encodeURIComponent(kbdId)}`
      : "home.html";
}
...
function irParaQuiz() {
  const marcaId = qs().get("marca");
  const kbdId = qs().get("kbd");
  if (!marcaId || !kbdId) return;
  window.location.href = `quiz.html?marca=${encodeURIComponent(marcaId)}&kbd=${encodeURIComponent(kbdId)}`;
}

function getPrimaryQuizHref(currentMarcaId, currentKbdId) {
  if (currentMarcaId && currentKbdId) {
    return `quiz.html?marca=${encodeURIComponent(currentMarcaId)}&kbd=${encodeURIComponent(currentKbdId)}`;
  }
  const next = getFirstPendingQuiz();
